"""
This part is taken from https://github.com/chifish/SyntacticGCN/tree/main
"""
    ...
    def forward(self, gcn_inp, adj_mat):
        ...
        for lbl in range(self.num_labels):
            if self.in_arcs:
                ...
                adj_matrix = adj_mat[:, lbl]  # Shape: [batch_size, len, len]
                in_act = torch.bmm(adj_matrix, inp_in)  # Aggregate: [batch_size, len, hidden_size]
            ...
            if self.out_arcs:
                ...
                adj_matrix = adj_mat[:, lbl]  # Shape: [batch_size, len, len]
                out_act = torch.bmm(adj_matrix, inp_out)  # Aggregate: [batch_size, len, hidden_size]
